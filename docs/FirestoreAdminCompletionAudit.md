...
| Emulator-backed real RPC behavior works | Firestore emulator integration tests cover CRUD, query, count, listen, listen reconnect after TCP drop, `limit(toLast:)` snapshot cursors, native GeoQuery, and opt-in Pipeline aggregation/DML/subquery smoke execution. | Verified locally |
