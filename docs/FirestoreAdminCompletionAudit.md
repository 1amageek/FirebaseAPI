...
|---|---|---|
| Server-side Admin facade exists | `Firestore` owns the public server workflow surface, and `FirestoreAdmin` is the preferred product import. The old `FirestoreAdmin` facade type has been removed. | Verified locally |
...
| Responsibility boundaries are split by dependency reason | Core, AuthCore, Auth, RuntimeConfig, Codable, AdminCore, AdminCodable, AdminGRPCBootstrap, Admin re-export, GeoQuery, Mongo, RPC, PipelineRPC, RuntimeSupport, Protobuf, GRPCStubs, and GRPCTransport are separate targets. | Verified locally |
| Public products are limited to intended adoption surfaces | `Package.swift` exposes only `FirestoreAdmin`, `FirestoreMongo`, and compatibility `FirestoreAPI` as public library products. | Verified locally |
| Public Admin import surface is curated | `FirestoreAdmin` re-exports exactly the server-side Admin modules and excludes Mongo-compatible query documents, RPC compilers, Pipeline RPC compilers, gRPC transport, generated protobuf, generated gRPC stubs, and runtime-support seams. | Verified locally |
| Compatibility import surface is curated | `FirestoreAPI` re-exports exactly the compatibility modules, including `FirestoreMongo` only for existing all-in-one imports, while excluding RPC compilers, Pipeline RPC compilers, gRPC transport, generated protobuf, generated gRPC stubs, and runtime-support seams. | Verified locally |
...
| MongoDB-compatible geospatial support is separate | `FirestoreMongo` owns `$near`, `$geometry`, GeoJSON, and `2dsphere` query/index documents; `FirestoreAdmin` does not re-export it. | Verified locally |
| Wasm Admin builds with an explicit host-transport boundary | `docs/FirestoreWasmBoundary.md` documents that `FirestoreAdmin` and `FirestoreAPI` build for WASI. Runtime execution requires a host-provided `GRPCCore.ClientTransport`; host-attached authentication is selected through `FirestoreSettings.hostManagedAuthentication(...)`, and `scripts/run-wasm-admin-smoke.sh` exercises a batch commit to the custom transport boundary. Direct NIO Posix transport use on WASI fails with an explicit transport error. | Verified locally |
...
| `bash scripts/check-wasm-compatible-targets.sh` | Passed with Swift 6.3.1 and `swift-6.3.1-RELEASE_wasm`; SwiftPM user-level cache warnings were non-fatal in the restricted environment |
