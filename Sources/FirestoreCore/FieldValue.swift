...
/**
 ...
 - `vector`: A vector embedding value that can be stored in a document field.
...
public enum FieldValue {
    ...
    /// An integer value that should be added to the existing value in the document field.
    case incrementInt64(Int64)

    /// Returns a sentinel value that deletes a field.
    public static func delete() -> FieldValue {
        .delete
    }

    /// Returns a sentinel value that sets a field to the server timestamp.
    public static func serverTimestamp() -> FieldValue {
        .serverTimestamp
    }

    /// Returns an integer increment transform.
    public static func increment(_ value: Int) -> FieldValue {
        .incrementInt64(Int64(value))
    }

    /// Returns an integer increment transform.
    public static func increment(_ value: Int64) -> FieldValue {
        .incrementInt64(value)
    }
...
public enum FieldValueEncodingError: Error {
    case encodingIsNotSupported(String)
}

extension FieldValue: Encodable {
    public func encode(to encoder: Encoder) throws {
        throw FieldValueEncodingError.encodingIsNotSupported("FieldValue values can only be encoded with FirestoreEncoder.")
    }
}
