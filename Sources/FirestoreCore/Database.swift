...
package struct Database: Codable, Equatable, Sendable {
     ...
     */
    package init(projectId: String, databaseId: String = "(default)") {
        self.projectId = projectId
        self.databaseId = databaseId
    }

    /// The path of the Firestore database.
    package var database: String { "projects/\(projectId)/databases/\(databaseId)" }

    /// The root path of the Firestore database.
    package var path: String { "projects/\(projectId)/databases/\(databaseId)/documents" }
}
