...
FirebaseAPI is shipped as a Swift package. `FirestoreAdminServer` is the preferred server-side Admin product, `FirestoreAPI` is the compatibility all-in-one import, `FirestoreMongoCore` is the explicit Mongo-compatible query-document product, and `FirestoreEmbedded` is the explicit dependency-free Embedded Swift descriptor product. Core model/query/snapshot values live in `Sources/FirestoreCore`, embedded-safe value/reference/query descriptors live in `Sources/FirestoreEmbedded`, Admin workflow code lives in `Sources/FirestoreAdmin`, Codable helpers live in `Sources/FirestoreCodable` and `Sources/FirestoreAdminCodable`, authentication lives in `Sources/FirestoreAuthCore` and `Sources/FirestoreAuth`, and runtime configuration lives in `Sources/FirestoreRuntimeConfig`. Native Firestore request compilation belongs in `Sources/FirestoreRPC`, Pipeline request compilation belongs in `Sources/FirestorePipelineRPC`, generated protobuf messages belong in `Sources/FirestoreProtobuf/Proto`, generated gRPC stubs belong in `Sources/FirestoreGRPCStubs/Proto`, and concrete grpc-swift transport execution belongs in `Sources/FirestoreGRPCTransport`. Tests mirror these responsibilities under `Tests/FirebaseAPITests`, with emulator and live smoke tests kept opt-in by environment. The upstream protobuf definitions are vendored under the `goolgeapis/` submodule; sync it before rebuilding generated code.

## Build, Test, and Development Commands
- `swift build --configuration debug` compiles the `FirestoreAPI` target and surfaces dependency issues early.
- `bash scripts/check-embedded-readiness.sh` verifies the dependency-free `FirestoreEmbedded` product with Embedded Swift enabled.
...
Follow the Swift API Design Guidelines: 4-space indentation, `UpperCamelCase` for types, and `lowerCamelCase` for members. Favor `struct` or `final class` to match existing patterns, and keep public API documented with concise doc comments. Extensions should stay feature-scoped. Avoid editing generated files or mixing concrete networking logic outside `Sources/FirestoreGRPCTransport`; public API targets must not import protobuf, generated gRPC stubs, or grpc-swift transport modules.
